...
На каждого пользователя в звонке нужно иметь 2 поток, на приём и передачу. Плюс раздача и прием аудио в разных микросервисах выполняется в 2 разных потоках. Это значит, что на 1 звонок у нас приходится `2 * 3(не все разговаривают) = 6` потоков. Максимально одновременне 1 млн. звонков, то есть 6 млн. потоков.
...
Итого на каждый звонок нужно иметь 8 потоков, 2,6 Мбайт оперативной памяти. По максимальному количеству звонков - 6 млн. потоков, 2535 Гбайт оперативной памяти. 96-ядерный процессор сможет обслужить 31 звонок, а значит потребуется 80,6 Мбайт оперативной памяти. Получается необходимо хост-серверов: 32258.

#### Т.е. в итоге получаем, что для обслуживания нашего пикового трафика нам необходимо 16129 серверов с характеристиками:
